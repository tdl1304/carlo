...
def setup_traffic_manager(traffic_manager: carla.TrafficManager, actor: carla.Actor, turns: int, percentage_speed_difference: int, path: Literal["left-loop", "city-wander", "straight"]):
    ...
    if path == "left-loop":
        traffic_manager.set_route(actor, ["Left"] * turns)
    elif path == "straight":
        traffic_manager.set_route(actor, ["Straight"])
    else:
        traffic_manager.set_route(actor, ["LaneFollow"])
        ...
        for index, run in enumerate(experiment.experiments):
            ...
            while not (should_stop(next_action, stop_next_straight, distance_traveled, run.stop_distance)):
                ...
                if (next_action == "Left" and previous_action != "Left"):
                    turns += 1
                    print("Turned left " + str(turns) + " times")
                    if (turns == run.turns):
            ...
            destroy_actors(session.world, "walker*")
